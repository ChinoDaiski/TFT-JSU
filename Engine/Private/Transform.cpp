...
_bool CTransform::Chase(_fvector vTargetPos, _double fTimeDelta, _double fLimitDistance)
{
	_vector		vPosition = Get_State(CTransform::STATE_POSITION);
	...
	{
		vPosition += XMVector3Normalize(vLook) * m_TransformDesc.SpeedPerSec * fTimeDelta;
		Set_State(CTransform::STATE_POSITION, vPosition);
		return true;
	}
...
void CTransform::Set_Speed(_double iSpeedLevel)
{
	m_TransformDesc.SpeedPerSec = iSpeedLevel * 1.0;
}
